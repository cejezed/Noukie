...
export class GoogleCalendarService {
    ...
    async getEvents(calendarId = 'primary', daysAhead = 7) {
        try {
            const calendar = google.calendar({ version: 'v3', auth: this.oauth2Client });
    ...
    convertEventToSchedule(event, userId) {
    ...
    createImportedEventRecord(event, scheduleId, userId) {
        return {
            userId,
            scheduleId,
            externalId: event.id,
