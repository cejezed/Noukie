...
// POST /api/chat/coach
// -----------------------------
export async function chatCoach(req, res) {
    try {
...
export async function explain(req, res) {
    try {
        const body = req.body ?? {};
        const raw = (typeof body.text === "string" ? body.text : body.message);
        const text = raw?.trim() ?? "";
        if (text.length < 3) {
        ...
        const subject = typeof body.subject === "string" && body.subject.trim().length
            ? body.subject.trim()
            : undefined;
        // Placeholder (vervang door je echte explain/LLM-call)
        const explanation = `Uitleg${subject ? ` (vak: ${subject})` : ""}:\n` +
