...
import { generatePlan, generateExplanation, expandExplanation, } from "./services/openai";
...
function parseRelativeDateTimeNL(inputRaw, now = new Date()) {
    const input = inputRaw.toLowerCase().trim();
    let date = startOfDay(now);
    let time = null;
    if (/\bmorgen\b/.test(input))
        date = startOfDay(addDays(now, 1));
    else if (/\bovermorgen\b/.test(input))
        date = startOfDay(addDays(now, 2));
    else if (/\bvandaag\b/.test(input))
        date = startOfDay(now);
    ...
    if (!time) {
        if (/\b(ochtend|smorgens)\b/.test(input))
            time = "09:00";
        else if (/\b(middag|vanmiddag)\b/.test(input))
            time = "14:00";
        else if (/\b(avond|vanavond)\b/.test(input))
            time = "19:00";
    }
    if (time) {
...
function datesHaveWeekdayInRange(startISO, endISO, weekday1to7) {
...
export async function registerRoutes(app) {
            ...
            if (shouldPlan) {
                ...
                for (const c of candidates) {
                    ...
                    if (courses?.length) {
                        ...
                        const hit = courses.find((k) => lower.includes(String(k.name).toLowerCase()));
                        if (hit)
                            course_id = hit.id;
                    }
    ...
    app.post("/api/courses", async (req, res) => {
        try {
            ...
            const created = await storage.createCourse(dbCourseData);
            res.json(created);
        }
        catch (error) {
    ...
    app.get("/api/schedule", async (req, res) => {
        try {
            const { userId, start, end, dayOfWeek } = req.query;
            if (!userId)
                return res.status(400).json({ error: "Missing userId" });
    ...
    app.post("/api/schedule", async (req, res) => {
        try {
            ...
            const dbScheduleData = {
                ...
                is_recurring: scheduleData.is_recurring ?? scheduleData.isRecurring ?? false,
            };
            // Normalize Sunday (0) → 7
            if (dbScheduleData.day_of_week === 0)
                dbScheduleData.day_of_week = 7;
            if (!dbScheduleData.user_id ||
            ...
            const created = await storage.createScheduleItem(dbScheduleData);
        ...
        try {
            ...
            for (const key in events) {
                ...
                if (summaryStr.includes("wiskundig") || summaryStr.includes("wiskunde"))
                    courseName = "Wiskunde";
                else if (summaryStr.includes("nederlands"))
                    courseName = "Nederlands";
                else if (summaryStr.includes("engels"))
                    courseName = "Engels";
                ...
                else if (summaryStr.includes("biologie"))
                    courseName = "Biologie";
                else if (summaryStr.includes("scheikunde"))
                    courseName = "Scheikunde";
                else if (summaryStr.includes("natuurkunde"))
                    courseName = "Natuurkunde";
                else if (summaryStr.includes("economie"))
                    courseName = "Economie";
                else if (summaryStr.includes("frans"))
                    courseName = "Frans";
                else if (summaryStr.includes("duits"))
                    courseName = "Duits";
                ...
                const isTest = summaryStr.includes("toets") ||
                    summaryStr.includes("test") ||
                    summaryStr.includes("exam") ||
    ...
    app.get("/api/parent/:parentId/children", async (req, res) => {
        try {
            ...
            const childrenData = await Promise.all(relationships.map(async (rel) => {
    ...
    app.get("/api/parent/child/:childId/mental-checkins", async (req, res) => {
        try {
            ...
            const checkinsMap = new Map(checkins.map(c => [c.date, {
                    date: c.date,
                    mood: c.mood,
