...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  /** ========= COMPLIMENTS ========= */
  // Helper function to check if user can send compliment today
  async function canSendComplimentToday(userId: string): Promise<boolean> {
    ...
    return (data?.length || 0) < 1; // Max 1 compliment per day
  }

  // Helper function to moderate message with OpenAI
  async function moderateMessage(message: string): Promise<{ safe: boolean; score: number }> {
    try {
      ...
      // Fail safe: if moderation fails, block the message
  ...
  async function updateStreak(userId: string): Promise<void> {
    ...
    if (!existingStreak) {
      // Create new streak
      await supabase.from("compliment_streaks").insert({
        user_id: userId,
        current_streak: 1,
        longest_streak: 1,
      ...
      const lastSentDate = existingStreak.last_sent_date;
      ...
      const newPoints = existingStreak.points + 5;

      // Calculate new badges
      const badges = [...(existingStreak.badges || [])];
      if (newStreak === 7 && !badges.includes("week_warrior")) {
        badges.push("week_warrior");
      }
      if (newStreak === 14 && !badges.includes("fortnight_friend")) {
        badges.push("fortnight_friend");
      }
      if (newStreak === 30 && !badges.includes("monthly_motivator")) {
        badges.push("monthly_motivator");
      }
      ...
      await supabase
        ...
        .update({
          ...
          last_sent_date: today,
          total_sent: existingStreak.total_sent + 1,
          points: newPoints,
          badges,
  ...
  // Helper function to increment received count
  async function incrementReceivedCount(userId: string): Promise<void> {
    const { data: existingStreak } = await supabase
      .from("compliment_streaks")
      .select("*")
      .eq("user_id", userId)
    ...
    if (!existingStreak) {
      ...
      await supabase
        .from("compliment_streaks")
        .update({
          total_received: existingStreak.total_received + 1,
          updated_at: new Date().toISOString(),
        })
        .eq("user_id", userId);
    }
  ...
  app.post("/api/compliments", async (req, res) => {
    try {
      ...
      const from_user = req.headers["x-user-id"] as string; // Get from auth middleware
      ...
      if (!to_user || !message) {
        return res.status(400).json({ error: "to_user and message are required" });
      }
      ...
      // Check daily limit
      const canSend = await canSendComplimentToday(from_user);
      if (!canSend) {
        return res.status(429).json({ error: "You can only send one compliment per day" });
      }
      ...
      if (!senderData?.classroom_id) {
        return res.status(400).json({ error: "You must be in a classroom to send compliments" });
      }

      // Verify recipient is in same classroom
      ...
      if (!recipientData || recipientData.classroom_id !== senderData.classroom_id) {
        return res.status(400).json({ error: "You can only send compliments to classmates" });
      }
      ...
      if (!moderation.safe) {
        return res.status(400).json({
          error: "Your message contains inappropriate content. Please try again with a positive, respectful compliment.",
          toxicity_score: moderation.score
        });
      }

      // Insert compliment
      const { data: compliment, error } = await supabase
        .from("compliments")
        .insert({
          from_user,
          to_user,
      ...
      if (error) {
        console.error("Error inserting compliment:", error);
        return res.status(500).json({ error: "Failed to send compliment" });
      }

      // Update sender's streak
      await updateStreak(from_user);

      // Update recipient's received count
      await incrementReceivedCount(to_user);

      res.json(compliment);
    } catch (error) {
      console.error("Compliments POST error:", error);
      res.status(500).json({ error: "Failed to send compliment" });
    }
  });

  // GET /api/compliments/mine - Get my received compliments
  app.get("/api/compliments/mine", async (req, res) => {
    try {
      ...
      console.error("Compliments GET error:", error);
      res.status(500).json({ error: "Failed to fetch compliments" });
    }
  });

  // GET /api/compliments/streak - Get my streak data
  app.get("/api/compliments/streak", async (req, res) => {
    try {
      const userId = req.headers["x-user-id"] as string;
  ...
  // GET /api/compliments/classmates - Get classmates in my classroom
  app.get("/api/compliments/classmates", async (req, res) => {
    try {
      const userId = req.headers["x-user-id"] as string;
      ...
      if (!userData?.classroom_id) {
        return res.json([]);
      }

      // Get all users in my classroom (except me)
      const { data, error } = await supabase
        .from("users")
      ...
      if (error) {
        ...
        return res.status(500).json({ error: "Failed to fetch classmates" });
      }

      res.json(data || []);
    } catch (error) {
      console.error("Classmates GET error:", error);
      res.status(500).json({ error: "Failed to fetch classmates" });
    }
  });

  // GET /api/compliments/stats - Get aggregate stats for teachers/classroom
  app.get("/api/compliments/stats", async (req, res) => {
    try {
      const userId = req.headers["x-user-id"] as string;
      ...
      // Get classroom stats
      const { data: complimentsData } = await supabase
        .from("compliments")
      ...
      const totalCompliments = complimentsData?.length || 0;
      ...
      const thisWeek = complimentsData?.filter(c => new Date(c.created_at) >= oneWeekAgo).length || 0;
      const thisMonth = complimentsData?.filter(c => new Date(c.created_at) >= oneMonthAgo).length || 0;

      res.json({
        total_compliments: totalCompliments,
        total_students: totalStudents,
        avg_per_student: totalStudents > 0 ? (totalCompliments / totalStudents).toFixed(1) : 0,
        this_week: thisWeek,
