...
export async function handleCreateScheduleItem(req: Request, res: Response) {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Niet geautoriseerd.' });
...
export async function handleDeleteScheduleItem(req: Request, res: Response) {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Niet geautoriseerd.' });
...
export async function handleCancelScheduleItem(req: Request, res: Response) {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Niet geautoriseerd.' });
