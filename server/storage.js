...
export class PostgresStorage {
    ...
    // Mental Checkins
    async getMentalCheckinsByStudentId(studentId, days = 30) {
    ...
    async getMentalMetrics(studentId) {
        ...
        checkins30d.forEach(c => {
            if (c.fun_with && c.fun_with.trim()) {
                const items = c.fun_with.split(',').map(s => s.trim()).filter(s => s);
                items.forEach(item => {
    ...
    async upsertMentalCheckin(checkin) {
        const { data, error } = await supabase
            .from("mental_checkins")
