...
// Voor auth.getUser(token) is ANON key voldoende; heb je service role, mag die ook
...
function buildSystemPrompt(mode, systemHint, context) {
...
export default function registerCoachRoute(router) {
    router.post("/api/coach", async (req, res) => {
        try {
            ...
            const mode = body.mode === "studeren" || body.mode === "chat" ? body.mode : "chat";
            if (!message)
