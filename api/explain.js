...
function scoreSignal(s) {
    ...
    if (/(->|→|⇌|≤|≥|\/)/.test(t))
        score += 1;
    if (/\b(tijdvak|oorzaak|gevolg|bron|revolutie|industrialisatie|middeleeuwen|verlichting)\b/.test(t))
        score += 3; // geschiedenis
    if (/\b(formule|vergelijking|functie|hoek|vector|differentieer|integreer|stelsel)\b/.test(t))
        score += 3; // wiskunde
    if (/\b(kracht|snelheid|versnelling|energie|impuls|veld|spanning|stroom)\b/.test(t))
        score += 3; // natuurkunde
    if (/\b(reactie|mol|molariteit|zuur|base|redox|stoichiometrie)\b/.test(t))
        score += 3; // scheikunde
    if (/\b(cel|osmose|mitose|enzym|homeostase|ecologie)\b/.test(t))
        score += 3; // biologie
    if (/\b(aanbod|vraag|elasticiteit|marginaal|bbp|inflatie)\b/.test(t))
        score += 3; // economie
    if (/(weet\s*ik\s*niet|idk|geen\s*idee|help\s*mij)/.test(t))
        score -= 3;
    if (/^\W+$/.test(t))
        score -= 2;
    return score;
}
function isLowSignal({ text, hasImage, hasPoging, historyLen }) {
    if (hasImage)
        return false;
    if (hasPoging)
        return false;
...
function normCourse(c) {
    ...
    if (/wiskunde|math/.test(t))
        return "wiskunde";
    if (/natuurkunde|physics/.test(t))
        return "natuurkunde";
    if (/scheikunde|chemie/.test(t))
        return "scheikunde";
    if (/biologie|biology/.test(t))
        return "biologie";
    if (/geschiedenis|history/.test(t))
        return "geschiedenis";
    if (/economie|economics/.test(t))
        return "economie";
    if (/nederlands|dutch/.test(t))
        return "nederlands";
    if (/engels|english/.test(t))
        return "engels";
    if (/frans|français/.test(t))
        return "frans";
    if (/duits|german/.test(t))
        return "duits";
    if (/aardrijkskunde|geografie|geography/.test(t))
        return "aardrijkskunde";
    return t || "algemeen";
}
function lowSignalReply(course, poging) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const generic = norm === "geschiedenis"
        ...
        const reply = `${norm === "algemeen" ? "" : `(${norm}) `}` +
