...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (path === "quizlet") {
    ...
    try {
      ...
      const { data: quiz, error: qErr } = await admin.from("study_quizzes").insert({ title, subject, chapter, description: description || null, user_id: userId, is_published: true }).select("*").single();
