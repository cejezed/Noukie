...
const violates = (s) => banned.some(rx => rx.test((s || "").toLowerCase()));
const fallback = () => "Daar kan ik niet mee helpen. Bespreek dit met je ouders of een leraar.";
const sanitize = (a) => (!a ? "Kun je je vraag over school of planning iets concreter maken?" : (a.length > 800 ? a.slice(0, 800) + " …" : a));
const detectMode = (t) => /(leg uit|uitleg|hoe werkt|samenvatting|oefening|begrijpen|verklaar)/i.test(t) ? "explain" : "plan";
async function sbSelectMemory(userId) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/coach_memory?user_id=eq.${userId}&limit=1`, {
        headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` }
    });
    if (!r.ok)
        throw new Error(`Supabase select failed: ${r.status} ${await r.text()}`);
    const rows = await r.json();
    return rows?.[0] ?? null;
}
async function sbUpsertMemory(payload) {
    ...
    if (!r.ok) {
        throw new Error(`Supabase upsert failed: ${r.status} ${await r.text()}`);
    }
}
...
function toISOFromLocalYYYYMMDD_HHMM(s) {
    const due = (s ?? "").trim();
    if (!due)
        return null;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (violates(message))
            return res.json({ mode: "blocked", answer: fallback() });
        ...
        console.error(e);
        return res.status(500).json({ error: "coach_error", detail: e?.message });
    }
}
