...
export default function Vandaag() {
    ...
    const addTaskMutation = useMutation({
        ...
        onSuccess: () => { qc.invalidateQueries({ queryKey: qcKey }); toast({ title: "Taak toegevoegd" }); },
        onError: (e) => { toast({ title: "Toevoegen mislukt", variant: "destructive", description: e?.message ?? "Onbekende fout" }); },
    });
    ...
    const onAddTask = (e) => {
        e.preventDefault();
        if (!title.trim())
            return toast({ title: "Titel is verplicht", variant: "destructive" });
    ...
    async function handleSend(e) {
        ...
        const text = msg.trim();
        if (!text)
            return toast({ title: "Leeg bericht", description: "Typ eerst je bericht.", variant: "destructive" });
        if (!coachRef.current?.sendMessage)
            return toast({ title: "Chat niet klaar", description: "CoachChat is nog niet geladen.", variant: "destructive" });
        try {
            setSending(true);
...
`.trim();
    return (_jsx("div", { className: "min-h-screen bg-slate-50", children: _jsxs("div", { className: "max-w-[1600px] mx-auto px-2 sm:px-3 md:px-4 py-3 md:py-4 space-y-4", children: [_jsxs("section", { className: "bg-white rounded-2xl border border-slate-200 p-3 md:p-4 space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("div", { className: "text-sm text-slate-600", children: "Praat met Noukie" }), _jsxs(Dialog, { children: [_jsx(DialogTrigger, { asChild: true, children: _jsx(Button, { variant: "ghost", size: "icon", className: "text-slate-400 hover:text-slate-600", children: _jsx(Info, { className: "h-5 w-5" }) }) }), _jsxs(DialogContent, { className: "rounded-2xl", children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { children: "Tips" }) }), _jsxs("div", { className: "space-y-3 text-sm text-slate-600 pt-1", children: [_jsx("div", { children: "Vraag om 1\u20132 concrete vervolgstappen." }), _jsx("div", { children: "Houd berichten kort; dan blijft de chat overzichtelijk." })] })] })] })] }), _jsx("div", { className: "w-full", children: _jsx(CoachChat, { ref: coachRef, systemHint: coachSystemHint, context: coachContext, size: "large", hideComposer: true, threadKey: `today:${userId || "anon"}` }) }), _jsxs("form", { onSubmit: handleSend, className: "space-y-2", children: [_jsx(Textarea, { placeholder: "Waarmee kan ik je helpen?", value: msg, onChange: (e) => setMsg(e.target.value), rows: 2, className: "min-h-[44px] text-base border-slate-200 focus:border-blue-300 focus:ring-blue-200 rounded-xl" }), _jsxs("div", { className: "flex flex-col sm:flex-row gap-2", children: [_jsx(SmartVoiceInput, { onTranscript: (text) => { setMsg(text); handleSend(); }, lang: "nl-NL" }), _jsxs(Button, { type: "submit", disabled: sending, className: "bg-slate-800 hover:bg-slate-700 rounded-xl", children: [sending ? _jsx(Loader2, { className: "w-4 h-4 mr-2 animate-spin" }) : null, sending ? "Versturen..." : "Stuur"] })] })] })] }), _jsxs("div", { className: "grid lg:grid-cols-2 gap-4 md:gap-6", children: [_jsxs("section", { className: "bg-white rounded-2xl border border-slate-200 p-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-3", children: [_jsx(Clock, { className: "w-5 h-5 text-slate-400" }), _jsx("h2", { className: "text-base font-medium text-slate-700", children: "Je rooster" })] }), scheduleLoading ? (_jsx("div", { className: "text-center py-8", children: _jsx(Loader2, { className: "w-6 h-6 animate-spin inline-block text-slate-400" }) })) : todayItems.length ? (_jsx("div", { className: "space-y-2", children: todayItems.map((item) => {
                                        const course = getCourseById(item.course_id);
                                        return (_jsxs("div", { className: "bg-slate-50 rounded-xl p-3 border border-slate-100", children: [_jsx("div", { className: "font-medium text-slate-700", children: item.title || course?.name || "Activiteit" }), _jsxs("div", { className: "text-sm text-slate-500 mt-0.5", children: [fmtTime(item.start_time), item.end_time ? ` - ${fmtTime(item.end_time)}` : ""] })] }, item.id));
                                    }) })) : (_jsx("div", { className: "text-center py-8 text-slate-400", children: "Geen activiteiten gepland" }))] }), _jsx(TasksPanel, { courses: courses, tasksToday: tasksToday, tasksLoading: tasksLoading, addTaskMutation: addTaskMutation, toggleDone: toggleDone, delTask: delTask, title: title, setTitle: setTitle, courseId: courseId, setCourseId: setCourseId, estMinutes: estMinutes, setEstMinutes: setEstMinutes, onAddTask: onAddTask })] })] }) }));
}
/** Takenpaneel losgetrokken voor leesbaarheid */
function TasksPanel(props) {
    const { courses, tasksToday, tasksLoading, addTaskMutation, toggleDone, delTask, title, setTitle, courseId, setCourseId, estMinutes, setEstMinutes, onAddTask } = props;
    return (_jsxs("section", { className: "bg-white rounded-2xl border border-slate-200 p-4", children: [_jsx("h2", { className: "text-base font-medium text-slate-700 mb-3", children: "Je taken" }), tasksLoading ? (_jsx("div", { className: "text-center py-8", children: _jsx(Loader2, { className: "w-6 h-6 animate-spin inline-block text-slate-400" }) })) : (_jsx("div", { className: "space-y-2", children: tasksToday.length === 0 ? (_jsx("div", { className: "text-center py-8 text-slate-400", children: "Geen taken voor vandaag" })) : (tasksToday.map((task) => {
                    const isDone = task.status === "done";
                    return (_jsxs("div", { className: "bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center justify-between gap-3", children: [_jsx("div", { className: `flex-1 text-sm ${isDone ? "line-through text-slate-400" : "text-slate-700"}`, children: task.title }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Button, { variant: "ghost", size: "icon", className: "h-8 w-8 text-slate-400 hover:text-green-600 hover:bg-green-50", onClick: () => toggleDone.mutate(task), children: _jsx(Check, { className: "w-4 h-4" }) }), _jsx(Button, { variant: "ghost", size: "icon", className: "h-8 w-8 text-slate-400 hover:text-red-600 hover:bg-red-50", onClick: () => delTask.mutate(task.id), children: _jsx(Trash2, { className: "w-4 h-4" }) })] })] }, task.id));
                })) })), _jsxs("div", { className: "mt-4", children: [_jsx("h3", { className: "text-sm font-medium text-slate-700 mb-2", children: "Nieuwe taak toevoegen" }), _jsxs("form", { onSubmit: onAddTask, className: "space-y-3", children: [_jsxs("div", { children: [_jsx(Label, { htmlFor: "t-title", className: "text-slate-600", children: "Wat moet je doen?" }), _jsx(Textarea, { id: "t-title", rows: 2, placeholder: "Bijv. Wiskunde \u00A72.3 oefenen, Engelse woordjes H2", value: title, onChange: (e) => setTitle(e.target.value), className: "mt-1.5 border-slate-200 focus:border-blue-300 focus:ring-blue-200 rounded-xl" })] }), _jsxs("div", { className: "grid sm:grid-cols-3 gap-3", children: [_jsxs("div", { children: [_jsx(Label, { htmlFor: "t-course", className: "text-slate-600", children: "Vak (optioneel)" }), _jsxs(Select, { value: courseId ?? "none", onValueChange: (v) => setCourseId(v === "none" ? null : v), children: [_jsx(SelectTrigger, { id: "t-course", className: "mt-1.5 border-slate-200 rounded-xl", children: _jsx(SelectValue, { placeholder: "Kies vak" }) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "none", children: "Geen vak" }), courses.map((c) => (_jsx(SelectItem, { value: c.id, children: c.name }, c.id)))] })] })] }), _jsxs("div", { children: [_jsx(Label, { htmlFor: "t-min", className: "text-slate-600", children: "Duur (min)" }), _jsx(Input, { id: "t-min", type: "number", min: 5, step: 5, placeholder: "30", value: estMinutes, onChange: (e) => setEstMinutes(e.target.value), className: "mt-1.5 border-slate-200 rounded-xl" })] }), _jsx("div", { className: "flex items-end", children: _jsxs(Button, { type: "submit", disabled: addTaskMutation.isPending, className: "w-full bg-slate-800 hover:bg-slate-700 rounded-xl", children: [addTaskMutation.isPending ? _jsx(Loader2, { className: "w-4 h-4 mr-2 animate-spin" }) : null, addTaskMutation.isPending ? "Toevoegen..." : "Toevoegen"] }) })] })] })] })] }));
}
