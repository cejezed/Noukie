...
const SCOPE_REGEX = new RegExp(`\\b(${[...SCHOOL_KEYWORDS, ...PLANNING_KEYWORDS]
    .map((k) => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
    .join("|")})\\b`, "i");
function isInScope(text) {
    ...
    if (!q)
        return false;
    if (q.length <= 2)
        return true; // laat "ok", "hi" door
    if (/(huiswerk|toets|examen|samenvat|oefen|paragraaf|hoofdstuk|vak|cijfer|docent|klas|les|rooster|planning|schema|studeren|leren|pomodoro|priori|taken?)/i.test(q)) {
        return true;
    }
    return SCOPE_REGEX.test(q);
}
const OUT_OF_SCOPE_REPLY = "Ik help je met school, leren en je planning. Waarmee kan ik je daarbij helpen?";
