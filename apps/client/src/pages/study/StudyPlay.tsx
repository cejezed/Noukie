  ...
  if (done || (list.length > 0 && index >= list.length)) {
    ...
    return (
      <main className="mx-auto max-w-[800px] px-6 py-8">
        ...
        <p className="mb-6">Score: <b>{correctCount}</b> / {list.length} ({pct}%)</p>
        ...
        choices.length ? (
          <div className="grid gap-3">
            {choices.map((c: string, i: number) => {
              ...
              const base = "text-left border rounded-xl p-3";
              const hover = showFb ? "" : " hover:bg-gray-50";
              const chosenWrong = isChosen && !isRight ? " border-red-400 bg-red-50" : "";
              const chosenRight = isChosen && isRight ? " border-emerald-500 bg-emerald-50" : "";
              const justRight = !isChosen && isRight ? " border-emerald-400" : "";
              const classes = base + hover + chosenWrong + chosenRight + justRight;
